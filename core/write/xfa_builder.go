...
func parseAllObjects(pdfBytes []byte, encryptInfo *types.PDFEncryption, verbose bool) (map[int]*PDFObject, error) {
	...
	if startXRef > 0 {
		xrefResult, err := parse.ParseXRefStreamFull(pdfBytes, startXRef, false)
		if err == nil {
			...
			for objNum, entry := range xrefResult.ObjectStreams {
				...
				objData, err := parse.GetObjectFromStream(pdfBytes, objNum, entry.StreamObjNum, entry.IndexInStream, encryptInfo, false)
				if err != nil {
					if verbose {
...
func findStartXRef(pdfBytes []byte) int64 {
	...
	searchLen := min(1024, len(pdfBytes))
...
func findXFAStreamMap(pdfBytes []byte, encryptInfo *types.PDFEncryption, verbose bool) (map[string]int, error) {
	...
	acroFormData, err := parse.GetObject(pdfBytes, acroFormObjNum, encryptInfo, false)
	if err != nil {

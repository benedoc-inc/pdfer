...
func AddActionToField(fieldObjNum int, action *Action, w *writer.PDFWriter) error {
...
func AddMouseAction(fieldObjNum int, event string, action *Action, w *writer.PDFWriter) error {
	// Get existing field object
	fieldData, err := w.GetObject(fieldObjNum)
	if err != nil {
		return fmt.Errorf("failed to get field object: %w", err)

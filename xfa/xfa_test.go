...
func TestFindXFADatasetsStream_Integration(t *testing.T) {
	...
	fileID := encryption.ExtractFileID(pdfBytes, false)
	encryptKey, err := encryption.DeriveEncryptionKey([]byte(""), encrypt, fileID, false)
	if err != nil {
	...
	xfaData, objNum, err := FindXFADatasetsStream(pdfBytes, encrypt, false)
	if err != nil {
